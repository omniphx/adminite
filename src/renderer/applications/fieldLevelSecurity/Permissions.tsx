...
const Permissions: React.FC = (props: any) => {
  ...
  const clearFieldPermissionsToSave = usePermissionUIStore(
    (state) => state.clearFieldPermissionsToSave
  );
  ...
  return (
    <div>
      <Row>
        ...
        <Col sm={24} md={12} style={{ textAlign: 'right' }}>
          ...
          >
            ...
            <Radio.Button value='permissionSet'>PermissionSet</Radio.Button>
          </Radio.Group>
        </Col>
      </Row>
      <Row>
        <Col sm={24} md={12}>
          <Select
            ...
            placeholder={
              permissionType == 'profile' ? 'Choose your Profiles' : 'Choose your Permission Sets'
            }
      ...
      <Row>
        <Col span={6}>
          <Input.Search placeholder='Filter fields' value={filter} onChange={handleChange} />
        </Col>
      </Row>
      <Row>
        <Col span={24}>
...
function getPermissions(permissions: any[]) {
  ...
  return permissions.map((permission) => {
    ...
    mapped.name = permission.IsOwnedByProfile ? permission.Profile?.Name : permission.Label;
