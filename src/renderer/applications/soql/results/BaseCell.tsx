...
const BaseCell: React.FC<IBaseCellProps> = (props: IBaseCellProps) => {
  ...
  const { fieldSchema, value, record, handleCancelEditMode, handleConfirmChange, editMode, setEditMode } = props
  ...
  function renderError() {
    return record.errorMessage && record.errorMessage.length > 0 && record.editFields.includes(fieldSchema.name)
      ? (
